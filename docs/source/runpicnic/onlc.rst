...
The simplest way to run PICNIC on the LC is directly from the command line. This works for small jobs that use a small number of processors and run for a short amount of time. In a directory with a valid PICNIC input file named input.txt, run using

.. code-block:: bash

   srun -N1 -n4 -ppdebug path/to/picnic.ex input.txt
...
.. code-block:: bash
   ...
   srun -N1 -n4 -ppdebug path/to/picnic.ex input.txt >> log.txt &
...
.. code-block:: bash
   ...
   setenv PICNIC /usr/workspace/angus1/codes/picnic/exec/picnic1d.Linux.64.mpicxx.mpif90.OPT.MPI.ex
   setenv NTASKS 8
