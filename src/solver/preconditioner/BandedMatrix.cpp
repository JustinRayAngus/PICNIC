...
void BandedMatrix::define(int a_nrow,int a_nbands,int a_istart)
{
  ...
  m_data        = (double*) calloc (m_nrow*m_nbands,sizeof(double));
...
void BandedMatrix::setRowValues(int a_row,int a_ncols, int *a_icols, double *a_data)
{
  ...
  double *data_ptr    = m_data  + row_local*m_nbands;
...
/* A <- aA */
void BandedMatrix::scaleEntries(double a_a)
{
  int i;
...
/* A <- aI + A */
void BandedMatrix::shift(double a_a)
{
  for (int i=0; i<m_nrow; i++) {
...
void BandedMatrix::getDiagonal(std::vector<double>& a_x) const
{
  a_x.clear();
  a_x.resize(m_nrow);
  for (int i=0; i<m_nrow; i++) {
...
void BandedMatrix::writeToFile(const std::string& a_fname, int a_op_flag)
{
  ...
  if (!procID()) {
    printf("%sBandedMatrix::writeToFile: Writing to sparse matrix file %s\n",
            m_op_indent.c_str(), a_fname.c_str() );
  }
  ...
  if (a_op_flag == 0) {
    ...
    for (int row=0; row<m_nrow; row++) {
      ...
      for (int i=0; i<m_ncols[row]; i++) {
        ...
        double val = (m_data+row*m_nbands)[i];
