...
double MathUtils::errorinv( const double&  a_x )
{
   ...
   if (std::abs(a_x)==1.0) {
      ...
      z = std::sqrt(-std::log((1.0+a_x)/2.0));
      ...
      soln = soln - (std::erf(soln) - a_x)/(2.0/std::sqrt(M_PI)*exp(-soln*soln));
      soln = soln - (std::erf(soln) - a_x)/(2.0/std::sqrt(M_PI)*exp(-soln*soln));
   }
