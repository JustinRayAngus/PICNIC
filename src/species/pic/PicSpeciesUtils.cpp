                        ...
                        const bool                    a_anticyclic )
{
   ...
   // advance vp from step n to step n+1 using Boris algorithm. 
   // vp = vpn + dt*q/m*(Ep(xpbar) + vpbar x Bp(xpbar))
...
void 
PicSpeciesUtils::applyForcesNew( List<JustinsParticle>&  a_pList,
                           const Real                    a_fnorm,
                           const Real                    a_cnormDt, 
                           const bool                    a_byWholeDt,
                           const bool                    a_anticyclic )
{
   if(a_pList.length()==0) return;   
   
   // compute vp at half time step n+1/2 using new method
   // vpbar = vpn + dt/2*q/m*(Ep(xpbar) + vpbar x Bp(xpbar))
   // if byWHoleDt is true, convert vpbar to vpnew: vp -> 2*vp - vpold

   // advance velocities using Boris algorithm
   Real bp0, bp1, bp2, denom;
   Real vm0, vm1, vm2, vmbp;
   Real alpha = a_fnorm*a_cnormDt/2.0;
   ...
   for(li.begin(); li.ok(); ++li) {
      ...
      vmbp = vm0*bp0 + vm1*bp1 + vm2*bp2;

      // compute vpbar = (vpnew + vpold)/2
