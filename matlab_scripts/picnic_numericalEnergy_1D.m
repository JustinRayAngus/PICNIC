...
tempTotX_2 = energyTotX_2 - momentumTotX_2.^2./numberTot_2/(me*Mass_2)/2.0;
tempTotY_2 = energyTotY_2 - momentumTotY_2.^2./numberTot_2/(me*Mass_2)/2.0;
tempTotZ_2 = energyTotZ_2 - momentumTotZ_2.^2./numberTot_2/(me*Mass_2)/2.0;
...
velXavg_1 = squeeze(mean(velX_1,1)); 
velYavg_1 = squeeze(mean(velY_1,1));
velZavg_1 = squeeze(mean(velZ_1,1));
%
velXavg_2 = squeeze(mean(velX_2,1)); 
velYavg_2 = squeeze(mean(velY_2,1));
velZavg_2 = squeeze(mean(velZ_2,1));
%
tempXavg_1 = squeeze(mean(tempX_1,1));
tempYavg_1 = squeeze(mean(tempY_1,1));
tempZavg_1 = squeeze(mean(tempZ_1,1));
%
tempXavg_2 = squeeze(mean(tempX_2,1));
tempYavg_2 = squeeze(mean(tempY_2,1));
tempZavg_2 = squeeze(mean(tempZ_2,1));

%
%%%   compute time vector [s] and nuei*t;
%

Tavg = (tempXavg_1(2) + tempYavg_1(2) + tempZavg_1(2))/3; % [eV]
...
subplot(1,2,1);
plot(time,velXavg_1,'displayName','x-velocity'); hold on;
plot(time,velYavg_1,'displayName','y-velocity'); hold on;
plot(time,velZavg_1,'displayName','z-velocity'); hold off;
xlabel('time [s]'); title('global average velocity');
ylabel('velocity [m/s]'); axis('tight');
legend('show','location','best');

subplot(1,2,2);
plot(time,tempXavg_1,'displayName','x-temperature'); hold on;
plot(time,tempYavg_1,'displayName','y-temperature'); hold on;
plot(time,tempZavg_1,'displayName','z-temperature'); hold off;
xlabel('time [s]'); title('global average temperature');
ylabel('temperature [eV]');
legend('show','location','best'); axis('tight');
tempAvg = (tempXavg_1+tempYavg_1+tempZavg_1)/3;
tempAvg0 = tempAvg(2);
